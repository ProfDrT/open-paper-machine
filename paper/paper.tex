...
The Open Academic Paper Machine (version 5.3.0) is an open-source plugin for Claude Code, Anthropic's AI-powered development environment. It transforms Claude from a general-purpose coding assistant into a specialized academic paper production agent by providing domain-specific knowledge through seven skill engines, external tool access through two MCP servers, and workflow orchestration through a structured pipeline with eight slash commands.
...
The system executes academic paper production through a six-phase pipeline with an optional seventh phase, each triggered by a specific slash command or executed sequentially through the primary \texttt{/write-paper} command.
...
\caption{The six-phase pipeline of the Open Academic Paper Machine with human quality-gate checkpoints. Each phase is executed by specialized skill engines and produces persistent artifacts. The optional seventh phase (citation verification) validates that cited sources support attributed claims.}
\label{fig:pipeline}
\end{figure}
...
\textbf{DP2: Skill Modularity.} \emph{Domain expertise should be encapsulated in modular, independently maintainable skill engines that the LLM loads on demand.} The seven-engine architecture separates concerns, enabling domain experts to contribute knowledge to specific engines without understanding the full system, and supporting recomposition for different use cases.
...
\midrule
DP1: Phase Decomposition & Manage complexity, enable oversight & 6+1 phase pipeline with checkpoints \\
DP2: Skill Modularity & Separation of concerns, extensibility & 7 independent skill engines \\
